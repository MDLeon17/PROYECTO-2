...
def resource_path(relative_path):
    """Funciona para acceso a recursos en .exe compilado con PyInstaller"""
    try:
        base_path = sys._MEIPASS  # carpeta temporal usada por PyInstaller
    except AttributeError:
        base_path = os.path.abspath(".")  # cuando corres el .py directamente
...
def menu_principal():
    ...
    IMAGEN_PATH = resource_path("logo_proyecto.png")

    ventana_ancho = 400
    ventana_alto  = 600
    ventana_principal.geometry(f"{ventana_ancho}x{ventana_alto}")

    # Tamaño máximo permitido de img
    max_w, max_h = 300, 250
    margen_inferior = 20 
    ...
    y_pos = ventana_alto - new_h - margen_inferior
